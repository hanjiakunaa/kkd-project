    ...
    >
      <n-grid :cols="24" :x-gap="24">
        ...
        <n-form-item-gi :span="12" path="name" :rule="required">
          <template #label>
            <question-label label="名称" content="标题" />
          </template>
          <n-input v-model:value="modalForm.name" />
        </n-form-item-gi>
        <n-form-item-gi :span="12" path="code" :rule="required">
          <template #label>
            <question-label label="编码" content="如果是菜单则对应前端路由的name，使用大驼峰" />
          </template>
        ...
        >
          <template #label>
            <question-label label="路由地址" content="父级菜单可不填" />
          </template>
        ...
        <n-form-item-gi v-if="modalForm.type === 'MENU'" :span="12" path="show">
          <template #label>
            <question-label label="显示状态" content="控制是否在菜单栏显示，不影响路由注册" />
          </template>
